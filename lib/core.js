...
var toc = function (title, modules) {
    return __spreadArrays([
        ...
        ''
    ], modules.map(function (m) { return "- [" + dropFirstDir(m).replace(/\.md$/, '') + "](/" + m.replace(/\.md$/, '') + ")"; })).join('\n')
        .trim();
};
...
var handleConfig = function (contents, navSections) {
    ...
    return __spreadArrays([before, 'nav:', '  - Overview: index.md', '  - Modules:'], navSections.map(navSection), ['', after]).join('\n')
        .trim();
};
var mkNav = function (dirs, files, modules) {
    ...
    return __spreadArrays([
        { type: 'file', title: 'Overview', path: 'index.md' },
        {
            type: 'dir',
            title: 'Modules',
            contents: modules.map(function (m) { return ({ title: dropFirstDir(m).replace(/\.md$/, ''), path: m }); })
        }
    ], navDirs, navFiles);
};
...
var dirsAndFiles = function (paths) { return function (_a) {
    var C = _a.C;
...
var readModules = function_1.pipe(readAllMdFiles('docs/modules'), RTE.map(function (files) { return files.map(relativeToDocs); }));
...
exports.main = function_1.pipe(removeDocsTsConfig, RTE.chain(function () {
    return Apply_1.sequenceS(RTE.readerTaskEither)({
        ...
        others: function_1.pipe(readOthers, RTE.chain(dirsAndFiles), RTE.chain(function (_a) {
    ...
    var modules = _a.modules, mkdocsConfig = _a.mkdocsConfig, others = _a.others;
    ...
    var newIndex = modulesIndex({ toc: toc('Table of contents', rest), allModules: toc('All modules', rest) });
