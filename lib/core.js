...
var directoryIndexFirst = Ord.fromCompare(function (a, b) {
    var dirnameA = path.dirname(a);
    var dirnameB = path.dirname(b);
    if (dirnameA === dirnameB) {
        var basenameA = path.basename(a);
        var basenameB = path.basename(b);
        if (basenameA === 'index.md')
            return -1;
        if (basenameB === 'index.md')
            return 1;
...
var toc = function (title, modules) {
    return __spreadArrays([
        ...
        ''
    ], function_1.pipe(modules, A.map(relativeToDocs), A.sort(directoryIndexFirst), A.map(function (m) {
        var label = dropFirstDir(m).replace(/\.md$/, '');
        var path = m.replace(/\.md$/, '').replace(/\/index$/, '');
        return "- [" + label + "](/" + path + ")";
    }))).join('\n')
        .trim();
};
...
var mkNav = function (dirs, files, modules) {
    ...
    return __spreadArrays([
        { type: 'file', title: 'Overview', path: 'index.md' },
        {
            ...
            ], modules.map(function (m) { return ({ title: dropFirstDir(relativeToDocs(m)).replace(/\.md$/, ''), path: relativeToDocs(m) }); }))
        }
    ], navDirs, navFiles);
};
...
var isMdFile = function (file) { return path.extname(file) === '.md'; };
...
exports.main = function_1.pipe(removeDocsTsConfig, RTE.chain(function () {
    return Apply_1.sequenceS(RTE.readerTaskEither)({
        ...
        others: function_1.pipe(readOthers, RTE.chain(splitByDirsAndFiles), RTE.chain(function (_a) {
            ...
            return Apply_1.sequenceS(RTE.readerTaskEither)({ dirs: dirsRefs(dirs), files: function_1.pipe(files, A.filter(isMdFile), mdFilesRefs) });
        })),
        modulesDirectory: function_1.pipe(readDirectory('docs/modules'), RTE.map(Tree.map(function (dir) { return (__assign(__assign({}, dir), { files: dir.files.filter(isMdFile) })); })), RTE.chainFirst(function (modulesDirectory) {
